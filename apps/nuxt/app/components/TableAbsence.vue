...
const employees = computed(() => {
  ...
  for (const a of props.absences) {
    ...
    if (!grouped.has(key)) {
      grouped.set(key, {abscenceId: a.abscenceId, firstName: a.firstName, lastName: a.lastName, personnelNumber: a.personnelNumber || '', absences: [] })
    }
...
<template>
    ...
    <div class=" border border-gray-300 rounded-lg overflow-x-auto max-h-[400px] overflow-y-auto">
        <table v-if="absences.length > 0" 
            class="min-w-full border-collapse text-sm text-left text-gray-700">
          <HeaderTable v-model="show" 
            :monthDates/>
          <BodyTable v-model="show" 
            :employees 
            :monthDates 
            @onSelect="onSelect"/>
        </table>
    ...
    <Modal v-model="show" 
      @onClose="show = false" 
      class="z-30">
      <FormAbsence v-model="absenceForm" 
        :action
        @onClose="show = false" 
